      ...
      <nav class="sticky top-0 z-50 bg-white/10 backdrop-blur-md border-b border-white/20 py-4">
        <div class="max-w-6xl mx-auto px-8 flex justify-between items-center">
          ...
          <div class="flex gap-8">
            <a routerLink="/home" class="text-white/80 hover:text-white hover:bg-white/10 px-4 py-2 rounded-lg transition-all duration-300 font-medium">Home</a>
            <a routerLink="/todos" class="text-white bg-white/10 px-4 py-2 rounded-lg font-medium">Todos</a>
          </div>
        </div>
      </nav>
      ...
      <div class="py-8 pb-16">
        <div class="max-w-4xl mx-auto px-8">
          <div class="text-center mb-12">
            <h1 class="text-5xl font-extrabold text-white mb-4 bg-gradient-to-r from-white to-indigo-100 bg-clip-text text-transparent">Todo List</h1>
            <p class="text-white/90 text-xl">Real-time task management powered by Convex</p>
          </div>

          <!-- Add Todo Form -->
          <div class="mb-8">
            <form (ngSubmit)="addTodo()" class="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6">
              <div class="flex gap-4 flex-col md:flex-row">
                ...
                >
                  ...
                  <span *ngIf="isAddingTodo()" class="opacity-70">Adding...</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <!-- Loading State -->
          <div *ngIf="isLoading()" class="text-center py-12 text-white bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl">
            <div class="w-10 h-10 border-3 border-white/30 border-t-white rounded-full animate-spin mx-auto mb-4"></div>
            <p>Loading todos...</p>
          </div>

          <!-- Error State -->
          <div *ngIf="error()" class="text-center py-12 text-white bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl">
          ...
          <div *ngIf="!isLoading() && !error()">
            <!-- Stats -->
            <div class="flex justify-center gap-8 mb-8 p-6 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl text-white" *ngIf="todos() && todos()!.length > 0">
            ...
            <!-- Empty State -->
            <div *ngIf="!todos() || todos()!.length === 0" class="text-center py-12 text-white bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl">
            ...
            <!-- Todo Items -->
            <div *ngIf="todos() && todos()!.length > 0" class="flex flex-col gap-3">
              <div
              ...
              >
                <div class="flex items-center gap-4 flex-1">
                  <button
                    ...
                    [class]="todo.done ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 border-emerald-500' : ''"
                    (click)="toggleTodo(todo)"
                    [disabled]="isUpdating(todo._id)"
                  >
                    <span *ngIf="todo.done" class="text-white font-bold text-sm">✓</span>
                  </button>
                  <span class="text-base transition-all duration-300" [class.line-through]="todo.done">
                    {{ todo.text }}
                  </span>
                </div>
                <div class="flex items-center gap-4">
                  <span class="text-sm opacity-70 whitespace-nowrap hidden sm:block">
                  ...
                  >
                    ...
                    <span *ngIf="isDeleting(todo._id)" class="opacity-70">⏳</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <!-- Quick Actions -->
            <div *ngIf="todos() && todos()!.length > 0" class="flex justify-center gap-4 mt-8 pt-8 border-t border-white/20 flex-col sm:flex-row items-center">
              <button
                *ngIf="pendingCount() > 0"
...
export class Todos {
  ...
  async deleteCompleted() {
    const completedTodos = this.todos()?.filter((todo: Todo) => todo.done) || [];
